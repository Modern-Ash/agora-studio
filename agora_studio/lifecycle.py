...
def normalize_lifecycle_query(query: Mapping[str, object] | None, *, detail: bool = False) -> dict[str, str]:
    values = query or {}
    allowed = {"swarm", "work", *(("revision",) if detail else ())}
    unknown = set(values) - allowed
    if unknown:
        raise LifecycleError("invalid_query", f"unknown lifecycle query field: {sorted(unknown)[0]}")
    normalized: dict[str, str] = {}
    for key in allowed:
        raw = values.get(key)
        if isinstance(raw, (list, tuple)):
            if len(raw) != 1:
                raise LifecycleError("invalid_query", f"lifecycle query field {key} must be provided once")
            raw = raw[0]
        ...
        if key in ("swarm", "work") and not _SLUG.fullmatch(raw):
            raise LifecycleError("invalid_query", f"lifecycle query field {key} must be a safe Agora slug")
...
def _gate_blockers(gate: dict[str, object], work: dict[str, object]) -> list[str]:
    ...
    if gate.get("require-all-criteria") is True:
        criteria = set((work.get("acceptance_criteria") or {}).keys()) if isinstance(work.get("acceptance_criteria"), dict) else set()
    ...
    if gate.get("require-successful-evidence") is True and "success" not in (work.get("evidence_results") or []):
...
def _method_projection(project: Path, method_id: str, work: dict[str, object]) -> tuple[dict[str, object], list[str]]:
    ...
    try:
        ...
        return ({
            "available": False, "id": method_id, "states": [], "transitions": [],
            "current_state": work.get("state"), "initial_state": None, "terminal_state": None,
        }, [f"Method topology unavailable: {error}"])
    states = method.get("work-states")
    if not isinstance(states, list) or not all(isinstance(value, str) and value for value in states):
        return ({
            "available": False, "id": method_id, "states": [], "transitions": [],
            "current_state": work.get("state"), "initial_state": None, "terminal_state": None,
    ...
    for index, path in enumerate(transition_paths):
        try:
            ...
            if (record.get("schema") != "agora/transition/v1" or source not in state_set or target not in state_set
            ...
            blockers = _gate_blockers(gates[gate_id], work) if gate_id in gates and source == current else []
            if source == current and work.get("operational_status") not in (None, "active"):
                reason = work.get("status_reason") if isinstance(work.get("status_reason"), str) else "no reason recorded"
                blockers.append(f"work is {work.get('operational_status')}: {reason}")
            transitions.append({
                "id": f"transition-{index}-{source}-{target}", "from": source, "to": target,
                "roles": roles, "gate": gate_id, "blockers": blockers,
                "available": source == current and not blockers, "source": f"repo://{path.relative_to(project).as_posix()}",
            })
        except (OSError, UnicodeError, ValueError):
            diagnostics.append(f"Ignored malformed transition record: {path.name}")
    incoming = {transition["to"] for transition in transitions}
    initial = next((state for state in states if state not in incoming), states[0] if states else None)
    ...
    return ({
        ...
        "states": [{
            "id": value, "initial": value == initial, "current": value == current,
...
def _activity_projection(events: list[dict[str, object]], current_state: object) -> dict[str, object]:
    ...
    for event in events:
        ...
        if event.get("type") == "work.transitioned" and "from" in fields and "to" in fields:
            traversals.append({
                "id": f"traversal-{len(traversals)}", "from": fields["from"], "to": fields["to"],
                "timestamp": event.get("timestamp"), "actor": event.get("actor"),
                "session_id": event.get("session_id"), "source": event.get("source"),
                "role": fields.get("role"),
            })
        event_type = str(event.get("type") or "")
        if "handed-off" in event_type or (event_type.startswith("session.") and event_type in ("session.failed", "session.prepared")):
            annotations.append({
                "id": f"annotation-{len(annotations)}", "type": event_type,
                "timestamp": event.get("timestamp"), "actor": event.get("actor"),
                "session_id": event.get("session_id"), "summary": event.get("summary"),
                "source": event.get("source"), "traversal_id": None,
            })
    return {"traversals": traversals, "annotations": annotations, "current_state": current_state}


def _state_at(revision_timestamp: object, traversals: list[dict[str, object]], initial: object, current: object) -> object:
    if revision_timestamp is None:
        return current
    state = initial
    for traversal in traversals:
        timestamp = traversal.get("timestamp")
        if isinstance(timestamp, str) and isinstance(revision_timestamp, str) and timestamp <= revision_timestamp:
            state = traversal.get("to")
    return state


def build_lifecycle(store: object, query: Mapping[str, object] | None, git: GitHistoryReader | None = None) -> dict[str, object]:
    ...
    if selection is None:
        raise LifecycleError("project_required", "Select a local Agora project before loading lifecycle data.")
    work_records = store._cli.execute("work", selection.path).data
    swarm_records = store._cli.execute("swarms", selection.path).data
    work = next((item for item in work_records if isinstance(item, dict) and item.get("id") == normalized["work"] and item.get("swarm_id") == normalized["swarm"]), None)
    if work is None:
        raise LifecycleError("not_found", "the selected work does not exist in the selected swarm")
    swarm = next((item for item in swarm_records if isinstance(item, dict) and item.get("id") == normalized["swarm"]), None)
    if swarm is None or not isinstance(swarm.get("method"), str) or not _SLUG.fullmatch(swarm["method"]):
    ...
    public_event_fields = ("timestamp", "type", "summary", "actor", "swarm_id", "work_id", "session_id", "tool_run_id", "source")
    ...
    try:
        ...
        for revision in specification["revisions"]:
            revision["work_state"] = _state_at(
                revision.get("timestamp"), activity["traversals"], method.get("initial_state"), work.get("state")
            )
            sha = revision.get("sha")
            revision["activity_refs"] = [
                {"type": event.get("type"), "source": event.get("source"), "actor": event.get("actor"), "session_id": event.get("session_id")}
                for event in public_events
                if isinstance(sha, str) and _summary_fields(event.get("summary")).get("commit") == sha
            ]
    except (OSError, UnicodeError, ValueError, GitReadError) as error:
        specification = {"available": False, "uri": None, "path": None, "revisions": [], "reason": str(error)}
    ...
    return {
        ...
        "availability": {
            "method": bool(method.get("available")), "activity": True,
            "specification": bool(specification.get("available")), "partial": bool(diagnostics),
        },
        "diagnostics": diagnostics,
    }


def build_revision_detail(store: object, query: Mapping[str, object] | None, git: GitHistoryReader | None = None) -> dict[str, object]:
    normalized = normalize_lifecycle_query(query, detail=True)
    projection = build_lifecycle(store, {"swarm": normalized["swarm"], "work": normalized["work"]}, git)
    ...
    if normalized["revision"] not in known:
        raise LifecycleError("not_found", "the requested revision is not available for this specification")
    selection = store.selection
    reader = git or GitHistoryReader()
    detail_payload = reader.detail(selection.path, specification["path"], normalized["revision"])
    return {"selection": selection.as_dict(), "scope": projection["scope"], "detail": detail_payload}
